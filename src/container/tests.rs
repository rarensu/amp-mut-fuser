...
mod t_u8 {
    ...
    fn container_deref_slice() {
        ...
        let borrowed = container.lock().unwrap();
    ...
    fn owned_variants() {
        ...
        assert_eq!(&*container_box.lock().unwrap(), data_box_orig.as_ref());
        ...
        #[cfg(not(feature = "no-rc"))]
        {
            ...
            assert_eq!(&*container_rc.lock().unwrap(), data_rc_orig.as_ref());
        }

        // Container::Arc
        let container_arc = Container::from(data_arc_orig.clone());
        assert_eq!(&*container_arc.lock().unwrap(), data_arc_orig.as_ref());

        #[cfg(not(feature = "no-rc"))]
        {
            // Container::RcBox
            let container_rc_box = Container::RcBox(Rc::new(data_box_orig.clone()));
            assert_eq!(&*container_rc_box.lock().unwrap(), data_box_orig.as_ref());

            // Container::RcVec
            let container_rc_vec = Container::RcVec(Rc::new(data_vec_orig.clone()));
            assert_eq!(&*container_rc_vec.lock().unwrap(), data_vec_orig.as_slice());
        }

        // Container::ArcBox
        let container_arc_box = Container::ArcBox(Arc::new(data_box_orig.clone()));
        assert_eq!(&*container_arc_box.lock().unwrap(), data_box_orig.as_ref());
    ...
    fn locking_variants_read() {
        ...
        match container_arc_mutex_box.lock() {
            Ok(guard) => assert_eq!(&*guard, data_box_orig.as_ref()),
        ...
        match container_arc_mutex_vec.lock() {
            Ok(guard) => assert_eq!(&*guard, data_vec_orig.as_slice()),
        ...
        match container_arc_rw_lock_box.lock() {
            Ok(guard) => assert_eq!(&*guard, data_box_orig.as_ref()),
        ...
        match container_arc_rw_lock_vec.lock() {
            Ok(guard) => assert_eq!(&*guard, data_vec_orig.as_slice()),
