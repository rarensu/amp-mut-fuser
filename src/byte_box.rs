...
/// from `Filesystem` trait methods like `read`. It allows filesystem
...
///
/// - `Borrowed`: For permanent, immutable data that can be safely borrowed (e.g., literals)
/// - `Owned`: For single-use data that is associated with the current request.
/// - `Shared`: For multi-use data that may be saved in memory or dropped, as needed.
#[derive(Debug)]
pub enum ByteBox<'a> {
    /// A borrowed slice of bytes. This variant should be used when the data
    /// can be safely borrowed for the lifetime `'a` (usually, 'static).
    Borrowed(&'a [u8]),
...
// ByteBox helper functions for your convenience. 
...
///
/// - `Borrowed`: For `&str` that can be borrowed (e.g., const or static).
...
pub enum OsBox<'a> {
    ...
    Borrowed(
        &'a str
        // &'a OsStr would have been preffered here, for the added safety.
        // Sadly, it is not currently feasible to create OsStr with a long enough lifetime 
        // to make a borrowed OsStr useful. For example, &'static OsStr is not supported. 
        // Recommend revisit this in the future if an alternative is discovered. 
    ),
    /// An owned `Box<OsStr>`.
    Owned(Box<OsStr>),
    /// A shared `Arc<OsStr>`.
...
// OsBox helper functions for your convenience

impl<'a> From<&'a str> for OsBox<'a> {
    fn from(s: &'a str) -> Self { OsBox::Borrowed(s) }
}
...
/// The `'name` lifetime is for the `name` field within the `DirEntryData`, if that name is borrowed.
#[derive(Debug, Clone)]
...
// DirEntryContainer helper functions, for your convenience
...
pub enum DirEntriesList<'dir, 'entry, 'name> {
    ...
    // 'entry is the lifetime of a borrowed entry: DirEntry.
    /// A borrowed slice of `DirEntryContainer`s. This may be used for a
    /// directory whose entries are all permanent and immutable. 
...
// DirEntriesList helper functions for your convenience
...
/// A simple alias for the (DirEntry, Entry) tuple that is needed for a dirplus() request.
///
...
///
/// The lifetimes are:
/// - `'entry`: Lifetime of the `DirEntryPlusData` tuple within the `Borrowed` variant.
/// - `'name`: Lifetime of borrowed names (`OsStr`) within the `DirEntryData` part of the tuple.
#[derive(Debug, Clone)]
pub enum DirEntryPlusContainer<'entry, 'name> {
    /// A borrowed `(DirEntryData<'name>, FuserEntry)` tuple.
    Borrowed(&'entry DirEntryPlusData<'name>),
    /// An owned `(DirEntryData<'name>, FuserEntry)` tuple.
    Owned(DirEntryPlusData<'name>),
    /// A shared, atomically reference-counted `(DirEntryData<'name>, FuserEntry)` tuple.
    Shared(Arc<DirEntryPlusData<'name>>),
}

// DirEntryPlusContainer helper functions for your convenience
...
pub enum DirEntryPlusList<'dir, 'entry, 'name> {
    /// A borrowed slice of `DirEntryPlusContainer`s. This may be for a
    /// directory whose contents are entirely permanent and immutable.
    Borrowed(&'dir [DirEntryPlusContainer<'entry, 'name>]),
    /// An owned, heap-allocated slice of `DirEntryPlusContainer`s. This is used when
    /// the list is created dynamically for the current request.
...
// DirEntryPlusList helper functions for your convenience
...
impl<'name> DirEntryPlusList<'_, '_, 'name> {
    ...
    /// Example usage:
    /// for (dir_entry, entry) in entries_plus_list.to_tuples_iter() {}
...
impl<'entry, 'name> From<Vec<(DirEntryData<'name>, crate::reply::Entry)>>
    ...
    fn from(vec: Vec<DirEntryPlusData<'name>>) -> Self {
        let boxed_slice = vec.into_iter()
...
mod tests_os_box {
    ...
    #[test]
    fn os_box_clone() {
        let static_str= "static_val";

        let b1 = OsBox::Borrowed(static_str);
        let b2 = b1.clone();
...
mod tests_dir_entry_containers {
    ...
    fn dir_entry_container_borrowed() {
        let name_str = "borrowed_entry_name";
        let entry_data = create_dir_entry_data(1, OsBox::Borrowed(name_str));
        let container = DirEntryContainer::Borrowed(&entry_data);
        assert_eq!(container.as_ref().ino, 1);
        assert_eq!(container.as_ref().name.as_ref(), name_str);
    }
    ...
    fn dir_entries_list_borrowed() {
        let name1_static ="entry1_static_name";
    ...
    fn dir_entries_list_owned_from_vec() {
        let name1_static="static_name_in_vec";
    ...
    fn dir_entries_list_shared_from_arc() {
        let name1_static = "shared_static_name";
    ...
    fn dir_entry_plus_list_borrowed() {
        let name1_static = "plus_entry1_static";
    ...
    fn dir_entry_plus_list_owned_from_vec() {
        let name1_static = "plus_vec_static_name";
