...
#[cfg(feature = "abi-7-11")]
pub use reply::ReplyPoll2;
#[cfg(target_os = "macos")]
pub use reply::ReplyXTimes2;
pub use reply::ReplyXattr2;
pub use reply::{Reply2 as Reply, ReplyAttr2, ReplyData2, ReplyEmpty2, ReplyEntry2, ReplyOpen2};
pub use reply::{
    ReplyBmap2, ReplyCreate2, ReplyDirectory2, ReplyDirectoryPlus2, ReplyIoctl2, ReplyLock2, ReplyLseek2,
    ReplyStatfs2, ReplyWrite2,
};
...
pub trait Filesystem {
    ...
    fn init2(&mut self, _req: &Request2<'_>, _config: &mut KernelConfig) -> Result<(), c_int> {
        Ok(())
    }
    ...
    fn lookup2(&mut self, _req: &Request2<'_>, parent: u64, name: &OsStr, reply: ReplyEntry2) {
        warn!(
    ...
    fn forget2(&mut self, _req: &Request2<'_>, _ino: u64, _nlookup: u64) {}

    /// Like forget, but take multiple forget requests at once for performance. The default
    /// implementation will fallback to forget.
    #[cfg(feature = "abi-7-16")]
    fn batch_forget2(&mut self, req: &Request2<'_>, nodes: &[fuse_forget_one]) {
        for node in nodes {
            self.forget2(req, node.nodeid, node.nlookup);
        }
    }

    /// Get file attributes.
    fn getattr2(&mut self, _req: &Request2<'_>, ino: u64, fh: Option<u64>, reply: ReplyAttr2) {
        warn!(
    ...
    /// Read symbolic link.
    fn readlink2(&mut self, _req: &Request2<'_>, ino: u64, reply: ReplyData2) {
    ...
    /// Remove a file.
    fn unlink2(&mut self, _req: &Request2<'_>, parent: u64, name: &OsStr, reply: ReplyEmpty2) {
        debug!(
    ...
    /// Remove a directory.
    fn rmdir2(&mut self, _req: &Request2<'_>, parent: u64, name: &OsStr, reply: ReplyEmpty2) {
        debug!(
    ...
    fn open2(&mut self, _req: &Request2<'_>, _ino: u64, _flags: i32, reply: ReplyOpen2) {
        reply.opened2(0, 0);
    }
    ...
    fn flush2(&mut self, _req: &Request2<'_>, ino: u64, fh: u64, lock_owner: u64, reply: ReplyEmpty2) {
        debug!(
    ...
    /// not the meta data.
    fn fsync2(&mut self, _req: &Request2<'_>, ino: u64, fh: u64, datasync: bool, reply: ReplyEmpty2) {
        debug!(
    ...
    fn opendir2(&mut self, _req: &Request2<'_>, _ino: u64, _flags: i32, reply: ReplyOpen2) {
        reply.opened2(0, 0);
    }
    ...
    fn statfs2(&mut self, _req: &Request2<'_>, _ino: u64, reply: ReplyStatfs2) {
        reply.statfs2(0, 0, 0, 0, 0, 512, 255, 0);
    }
    ...
    /// If `size` is 0, the size of the value should be sent with `reply.size2()`.
    /// If `size` is not 0, and the value fits, send it with `reply.data2()`, or
    /// `reply.error2(ERANGE)` if it doesn't.
    fn getxattr2(
        &mut self,
        _req: &Request2<'_>,
        ino: u64,
    ...
    /// If `size` is not 0, and the value fits, send it with `reply.data2()`, or
    /// `reply.error2(ERANGE)` if it doesn't.
    fn listxattr2(&mut self, _req: &Request2<'_>, ino: u64, size: u32, reply: ReplyXattr2) {
        debug!(
    ...
    fn removexattr2(&mut self, _req: &Request2<'_>, ino: u64, name: &OsStr, reply: ReplyEmpty2) {
        debug!(
    ...
    fn access2(&mut self, _req: &Request2<'_>, ino: u64, mask: i32, reply: ReplyEmpty2) {
    ...
    fn bmap2(&mut self, _req: &Request2<'_>, ino: u64, blocksize: u32, idx: u64, reply: ReplyBmap2) {
        debug!(
    ...
    #[cfg(target_os = "macos")]
    fn setvolname2(&mut self, _req: &Request2<'_>, name: &OsStr, reply: ReplyEmpty2) {
    ...
    #[cfg(target_os = "macos")]
    fn getxtimes2(&mut self, _req: &Request2<'_>, ino: u64, reply: ReplyXTimes2) {
