...
use fuser::{
    ...
    Attr, ByteBox, DirEntriesList, DirEntryContainer, DirEntryData, Entry, Errno, 
    FileAttr, Filesystem, FileType, MountOption, Open, OsBox, RequestMeta, 
    FUSE_ROOT_ID,
};
...
impl Filesystem for FSelFS {
    ...
    fn readdir<'dir, 'entry, 'name>(
        ...
        for idx in start_offset..NUMFILES {
            ...
            entries.push(DirEntryContainer::Owned(entry_data));
            // TODO: compare to _max_bytes; stop if full.
        }
        // convert the vector of entries to an appropriate type
        // using a From<...>/Into<...> helper method
        Ok(entries.into())
    }
    ...
    fn read<'a>(
        ...
        *cnt -= size;
        ...
        // example of converting to ByteBox using explicit syntax
        Ok(ByteBox::Owned(data.into_boxed_slice()))
    }
